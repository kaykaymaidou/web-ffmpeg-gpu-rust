...
## 🏛️ 双轮驱动整体架构全景 (Dual-Engine Topography)
...
## 🚀 方向一：全面 Web 浏览器引擎（利用好 WebCodecs + WebGPU + WebRTC）
...
## 🛠️ 方向二：通用底层 Native SDK（独立、纯原生、对标标准 FFmpeg）
...
通过将核心算力收敛在 **`crates/core`**，本项目成功实现：
1. **一份核心代码，双向极致输出**：既能以 WASM + WebGPU + WebCodecs 统治前端高画质、低延迟多媒体交互；又能以纯原生 C-ABI 在桌面端与服务端作为对标 FFmpeg 的轻量级高性能引擎。
2. **拒绝单一依赖**：用户在浏览器内享受即点即用的零拷贝硬解，在桌面端或通过 Agent 调度时享受毫秒级自愈与满血原生转码，真正实现工业级通用化覆盖。
