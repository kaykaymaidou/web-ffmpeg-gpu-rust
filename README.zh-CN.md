...
# 产出 packages/core/pkg（~128KB）
npm run build:wasm
```

**Windows 注意**：若用户名含非 ASCII 字符，mingw `ld` 会找不到 sysroot。先跑一次：

```powershell
. .\scripts\env-rust-windows.ps1   # 把工具链镜像到 C:\Rust\rustup
npm run build:wasm
```

或装齐 **VS 2022 Build Tools + Windows SDK**，改用 `stable-x86_64-pc-windows-msvc`。

Playground 会通过 `loadRustCore()` 加载 `RustDemuxer` / `RustWasmMp4Muxer` / `RustCpuFilter`。
